...
int mutex_init(mutex* m) {
  ...
  m->handle =
      CreateMutex(NULL, FALSE, NULL); /* default security, not owned by calling thread, unnamed. */
  if (NULL == m->handle) {
    return -2;
  }
  return 0;
...
int thread_join(thread* t) {
  ...
  if (0 != pthread_join(t->handle, NULL)) {
    return -2;
  }
  return 0;
}
...
/* Generated font tables, kept one per line. */
/* clang-format off */
...
/* clang-format on */
static int fill(video_generator* gen, uint32_t x, uint32_t y, uint32_t w, uint32_t h, uint8_t r,
                uint8_t g, uint8_t b);
...
static void*
audio_thread(void* gen); /* When we need to generate audio, we do this in another thread. So be
                            aware that the callback will be called from this thread! */
...
int video_generator_init(video_generator_settings* cfg, video_generator* g) {
  ...
  if (!cfg->bitdepth) {
    cfg->bitdepth = DEFAULT_BITDEPTH;
  }
  if (!cfg->byte_order) {
    cfg->byte_order = DEFAULT_BYTE_ORDER;
  }
  if (!cfg->onecolor) {
    cfg->onecolor = 0;
  }
  ...
  if (NULL != cfg->audio_callback) {
    ...
    g->audio_nseconds = 4;
    g->audio_nbytes =
        sizeof(int16_t) * g->audio_samplerate * g->audio_nchannels * g->audio_nseconds;
    ...
    for (i = g->audio_samplerate; i < (g->audio_samplerate + num_frames); ++i) {
      dx = i * 2;
      g->audio_buffer[dx + 0] = (int16_t)(10000 * sin((6.28318530718 / g->audio_samplerate) *
                                                      g->audio_bip_frequency * i));
...
int video_generator_update(video_generator* g) {
  ...
  if (nlines + start_y > (int32_t)g->height || nlines < 0 || start_y < 0 ||
      start_y >= (int32_t)g->height) {
    printf("Error: this shouldn't happen.. writing outside the buffer: %d, %d, %d\n", nlines,
           (nlines + start_y), start_y);
    return -1;
  }
  ...
  for (i = (uint32_t)start_y; i < (uint32_t)(start_y + nlines); ++i) {
    if (g->pixel_size_in_bytes == 2) {
      for (k = 0; k < stride; k += g->pixel_size_in_bytes) {
  ...
  if (g->width > text_w && g->height > text_h) {
    ...
    /* Pass the pixel column, add_char() applies pixel_size_in_bytes for the byte offset. */
...
static int add_char(video_generator* gen, video_generator_char* kar, uint32_t x, uint32_t y) {
  ...
  for (i = kar->x, dest_x = x; i < (kar->x + kar->width); ++i, ++dest_x) {
    for (j = kar->y, dest_y = y; j < (kar->y + kar->height); ++j, ++dest_y) {
      ...
      /* Scale the 8-bit font intensity into the coded range (pixel_factor is 1/4/16 for
         8/10/12-bit) and write it as LSB/MSB */
      yc = (uint16_t)(pixels[src_dx] * gen->pixel_factor);
      dest_dx = (kar->yoffset + dest_y) * gen->width * gen->pixel_size_in_bytes +
                dest_x * gen->pixel_size_in_bytes;
      if (gen->pixel_size_in_bytes == 2) {
        gen->y[dest_dx] = LSB(yc, gen->byte_order);
        gen->y[dest_dx + 1] = MSB(yc, gen->byte_order);
      } else {
        gen->y[dest_dx] = (uint8_t)yc;
      }
    }
  }
